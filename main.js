...
function startAdapter(options) {
    ...
    if (id && id.channel && channels[id.channel]) {
      ...
      if (player) {
        if (id.state === "state_simple") {
          ...
          promise = player.timeSeek(
            Math.round((channels[id.channel].duration * state.val) / 100)
          );
        ...
        adapter.log.warn(
          `SONOS "${id.channel}"/"${channels[id.channel].uuid}" not found`
        );
        discovery.players.forEach((player) =>
          adapter.log.debug(
            `UUID: ${player.uuid} in ${player.roomName} / ${player.baseUrl}`
          )
        );
      }
    }
  ...
  adapter.on("unload", (callback) => {
    try {
      if (adapter) {
        adapter.setState &&
          aliveIds.forEach((id) => adapter.setState(id, false, true));
...
function enumName2Id(enums, name) {
  ...
  for (let e = 0; e < enums.length; e++) {
    if (
      enums[e] &&
      enums[e].value &&
      enums[e].value.common &&
      enums[e].value.common.name
    ) {
      if (typeof enums[e].value.common.name === "object") {
        ...
        if (
          enums[e].value.common.name &&
          enums[e].value.common.name.toLowerCase() === name
        ) {
          return enums[e].id;
        }
      }
    ...
    if (enums[e].value && enums[e].value.name) {
      if (typeof enums[e].value.name === "object") {
        for (const lang in enums[e].value.name) {
          if (
            enums[e].value.name.hasOwnProperty(lang) &&
            enums[e].value.name[lang].toLowerCase() === name
          ) {
            return enums[e].id;
          }
        }
...
async function createChannel(name, ip, room) {
  ...
  await adapter.setForeignObjectAsync(
    `${adapter.namespace}.root.${id}.cover_png`,
    {
  ...
  if (fileName && fileName.match(/^https?:\/\//)) {
    ...
    if (discovery) {
      ...
      for (let i = 0; i < discovery.players.length; i++) {
        ...
        setTimeout(
          () => playOnSonos(fileName, discovery.players[i].uuid, volume),
          100
        );
      }
    }
    ...
    if (!adapter.config.webServer) {
      return adapter.log.warn(
        "Web server must be enabled to play local TTS files"
      );
    }
            ...
            if (err) {
              ...
              adapter.getForeignObject(adapter.config.webServer, (err, obj) => {
                if (obj && obj.native && discovery) {
                  fileName = `http${obj.native.secure ? "s" : ""}://${
                    discovery.localEndpoint
                  }:${obj.native.port}/state/${id}`;
                  if (sonosIp) {
                    sonosIp = sonosIp.replace(/[.\s]+/g, "_");
                  }
                  ...
                  for (let i = 0; i < discovery.players.length; i++) {
                    if (!discovery.players[i]._address) {
                      discovery.players[i]._address = getIp(
                        discovery.players[i]
                      );
                    }
...
function takeSonosState(ip, sonosState) {
  adapter.setState(
    { device: "root", channel: ip, state: "alive" },
    { val: true, ack: true }
  );
  ...
  adapter.log.debug(
    `>  playbackState: ${sonosState.playbackState} - ${
      sonosState.currentTrack && sonosState.currentTrack.title
        ? sonosState.currentTrack.title
        : ""
    }`
  );
  ...
  if (!ps.transitioning) {
    adapter.setState(
      { device: "root", channel: ip, state: "state_simple" },
    ...
    if (ps.playing && channels[ip].duration > 0) {
      ...
      if (!channels[ip].elapsedTimer) {
        channels[ip].elapsedTimer = setInterval(
          (ip_) => {
            channels[ip_].elapsed +=
              (adapter.config.elapsedInterval || 5000) / 1000;
            ...
            adapter.setState(
              { device: "root", channel: ip, state: "seek" },
              {
                val:
                  Math.round(
                    (channels[ip_].elapsed / channels[ip_].duration) * 1000
                  ) / 10,
                ack: true,
              }
            );
  ...
  if (lastCover !== sonosState.currentTrack.albumArtUri) {
    ...
    if (sonosState.currentTrack.albumArtUri) {
      md5url = crypto
        .createHash("md5")
        .update(sonosState.currentTrack.albumArtUri)
        .digest("hex");
  ...
  if (channels[ip].duration > 0) {
    ...
    adapter.setState(
      ...
      {
        val:
          Math.round((channels[ip].elapsed / channels[ip].duration) * 1000) /
          10,
        ack: true,
      }
    );
  ...
  adapter.setState(
    ...
    { val: sonosState.volume, ack: true }
  );
...
function takeSonosFavorites(ip, favorites) {
  ...
  adapter.setState(
    { device: "root", channel: ip, state: "favorites_list" },
    { val: sFavorites, ack: true }
  );
  adapter.setState(
  ...
  if (event === "topology-change") {
    ...
    for (let i = 0; i < discovery.players.length; i++) {
      if (discovery.players[i].roomName === data.roomName) {
        ...
        if (channels[ip]) {
          ...
          adapter.log.debug(
            `group-volume: Volume for ${player.baseUrl}: ${data.newVolume}`
          );
        }
      }
    }
    ...
    if (player) {
      ...
      if (channels[ip]) {
        ...
        adapter.setState(
          { device: "root", channel: ip, state: "muted" },
          { val: data.newMute, ack: true }
        );
        //adapter.setState({device: 'root', channel: ip, state: 'muted'},  {val: data.groupState.mute,  ack: true});
        //player._isMuted = data.groupState.mute;
        player._isMuted = data.newMute;
        adapter.log.debug(
          "mute: Mute for " + player.baseUrl + ": " + data.newMute
        );
        adapter.setState(
          {
            device: "root",
        ...
        adapter.log.debug(
          "group_muted: groupMuted for " +
            player.baseUrl +
            ": " +
            player.groupState.mute
        );
      }
    }
    //            }
    ...
    if (player) {
      ...
      if (channels[ip]) {
        ...
        player._volume = data.newVolume;
        adapter.log.debug(
          `volume: Volume for ${player.baseUrl}: ${data.newVolume}`
        );
    ...
    if (player) {
      ...
      if (channels[ip]) {
        ...
        for (let q = 0; q < data.queue.length; q++) {
          ...
          _html.push(`
                        <tr class="sonosQueueRow" onclick="vis.setValue('${
                          adapter.namespace
                        }.root.${player._address}.current_track_number', ${
            q + 1
          })">
                        <td class="sonosQueueTrackNumber">${q + 1}</td>
                        <td class="sonosQueueTrackCover"><img src="${
                          player.baseUrl
                        }${data.queue[q].albumArtUri}"></td>
                        <td class="sonosQueueTrackArtist">${
                          data.queue[q].artist
                        }</td>
                        <td class="sonosQueueTrackAlbum">${
                          data.queue[q].album
                        }</td>
                        <td class="sonosQueueTrackTitle">${
                          data.queue[q].title
                        }</td>
                        </tr>
                        `);
        ...
        adapter.setState(
          { device: "root", channel: ip, state: "queue" },
          { val: qtext, ack: true }
        );
        adapter.log.debug(`queue for ${player.baseUrl}: ${qtext}`);
        adapter.setState(
          { device: "root", channel: ip, state: "queue_html" },
          { val: qhtml, ack: true }
        );
    ...
    adapter.log.debug(
      `${event} ${typeof data === "object" ? JSON.stringify(data) : data}`
    );
  }
}
...
async function updateHtmlQueue(player, trackNumber) {
  ...
  adapter.log.debug(
    `Update html-queue for ${player}: current html-queue is ${queue.val}`
  );

  //Remove old highlighting
  queue = queue.val.replace(
  ...
  adapter.log.debug(
    `Update html-queue for ${player}: current track number is ${trackNumber}`
  );
  ...
  adapter.log.debug(
    `Update html-queue for ${player}: RegEx pattern is ${regexPattern}`
  );
  ...
  adapter.log.debug(
    `Update html-queue for ${player}: got match ${currentTrack}`
  );
  ...
  adapter.log.debug(
    `Update html-queue for ${player}: new html string for current track is ${currentTrackHighlight}`
  );
...
async function syncConfig() {
  ...
  if (devices && devices.length) {
    ...
    for (let i = 0; i < devices.length; i++) {
      const _channels = await adapter.getChannelsOfAsync(
        devices[i].common.name
      );
      ...
      if (_channels) {
        adapter.log.debug(
          `Channels of ${devices[i].common.name}: ${JSON.stringify(_channels)}`
        );
            ...
            for (let u = 0; u < adapter.config.devices.length; u++) {
              if (adapter.config.devices[u].ip === ip) {
                ...
                if (adapter.config.devices[u].room) {
                  ...
                  try {
                    await adapter.deleteChannelFromEnumAsync(
                      "room",
                      "root",
                      id
                    );
                    ...
                    adapter.log.error(
                      "Cannot delete channel from enum: " + err.message
                    );
                  }
                }
              }
      ...
      if (configToAdd.length) {
        for (let r = 0; r < adapter.config.devices.length; r++) {
          if (
            ...
            configToAdd.includes(adapter.config.devices[r].ip)
          ) {
...
function main() {
  ...
  syncConfig().then(() => {
    cacheDir =
      path.join(utils.getAbsoluteDefaultDataDir(), "sonosCache") + path.sep;
